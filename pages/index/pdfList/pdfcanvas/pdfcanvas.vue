...
export default {
	...
	methods: {
		...
		getCode() {
			...
			getAppletCode({ pagePath: 'pages/index/pdfList/pdfDetail/pdfDetail', scene: this.pdfId }).then(res => {
				if (res.code == 0) {
					Promise.all([this.onlineToLocal(this.$imgUrl2 + res.data.fid, 1), this.onlineToLocal(this.$imgUrl2+this.logo, 2), this.onlineToLocal(this.headerImg, 3)])
						.then(result => {
